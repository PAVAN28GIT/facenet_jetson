...
for train_index, test_index in rkf.split(X):
    X_train_cv, X_test_cv = EMBEDDED_X[train_index], EMBEDDED_X[test_index]
    Y_train_cv, Y_test_cv = Y[train_index], Y[test_index]
    
    model.fit(X_train_cv, Y_train_cv)
    
    # Evaluate accuracy on training and testing sets
    train_accuracy = model.score(X_train_cv, Y_train_cv)
    test_accuracy = model.score(X_test_cv, Y_test_cv)
    precision = precision_score(Y_test_cv, model.predict(X_test_cv), average='weighted')
